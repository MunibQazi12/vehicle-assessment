...
function transformDealer(dealer: DealerInfo): TransformedDealer {
  ...
  return {
    ...
    serviceUrl: dealer.site_url ? `https://${dealer.site_url}/service` : "#",
    partsUrl: dealer.site_url ? `https://${dealer.site_url}/parts` : "#",
    mapUrl,
    brands: standardizeBrandNames(dealer.manufactures || []),
    dealerWebsite: dealer.site_url ? `https://${dealer.site_url}` : "#",
  }
}
...
export default async function Dealers() {
  ...
  const allStates = Array.from(
    new Set(dealers.map((dealer) => dealer.state).filter(Boolean))
  ).sort()
  ...
  return (
    <main>
      ...
      <PageHero
        title="Dealers"
        subtitle="FIND YOUR NEAREST DEALERSHIP"
        backgroundImage="/assets/images/dealers-hero.jpg"
      />
